...
const ResourcePage: React.FC = () => {
  ...
  const handleGenerateSummary = async () => {
    ...
    setError('');
    try {
      const result = await summarizeText(resource.content);
      setSummary(result);
    } catch (err) {
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mt-10 border-t dark:border-slate-700 pt-8">
          ...
          <button
            ...
            className="bg-brand-orange hover:bg-orange-600 text-white font-bold py-2 px-6 rounded-lg transition-colors duration-300 disabled:bg-slate-400 disabled:cursor-not-allowed flex items-center gap-2"
          >
            {isLoading ? 'Generating...' : 'Generate Summary'}
          </button>
          {!canSummarize && <p className="text-sm text-slate-500 dark:text-slate-400 mt-2">Summarization is available once the story has been published.</p>}
          ...
          {summary && (
            <div className="mt-6 p-6 bg-blue-50 dark:bg-slate-800/50 border-l-4 border-brand-blue rounded-r-lg animate-fade-in">
              <p className="text-slate-800 dark:text-slate-200 whitespace-pre-wrap">{summary}</p>
            </div>
          )}
        </div>
